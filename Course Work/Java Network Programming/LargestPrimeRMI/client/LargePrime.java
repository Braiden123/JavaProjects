...
public class LargePrime implements Task<Integer>, Serializable{

    private int largestPrime;
    private final int range;

    public LargePrime(int range) {
        this.range = range;
    }

    public Integer execute() {
        return computeLargePrime(range);
    }

    private Integer computeLargePrime(int range) {
        if (range <= 1) {
            return -1;
        }
        else if(range <= 3) {
        ...
        for (int i = 2; i <= range; i++) {
            int divCount = 2; //Already counting 1 and i.
